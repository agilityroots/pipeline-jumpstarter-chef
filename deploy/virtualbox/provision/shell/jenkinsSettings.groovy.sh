if [ ! -f jenkinsSettings.groovy ]
then
    cat > jenkinsSettings.groovy <<_EOF_
...
_EOF_
timeout 120 bash -c -- 'while [ $(curl --write-out %{http_code} --silent --output /dev/null http://localhost:8080/api/json) -ne 200 ]; do echo "." > /dev/null;done'
curl -sS --data-urlencode "script=$(<./jenkinsSettings.groovy)" http://localhost:8080/scriptText
fi